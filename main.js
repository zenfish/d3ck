...
function cat_power(msg) {
    ...
    // pushes to both browsers if it can
...
function create_cli3nt_rest(req, res, next) {
    ...
    else if (typeof secrets2d3cks[secret] !== "undefined") {
        log.info('loading did from secret lookup')
        did = secrets2d3cks[secret]
    }
    ...
    // is there a secret in here?  Although... if you're the auth'd client... no worries, you go by
        ...
        if (typeof secret_requests[did] === "undefined") {
...
//
//  /setTCPproxy?proxy_remote_host=10.0.0.1&proxy_remote_port=22&proxy_local_port=6666
//
// And then connections to 192.168.0.1 on port 6666 will be
// redirected to port 22 on the host 10.0.0.1
//
function setTCPproxy(req, res, next) {

    res.send(200, {msg: "not implemented yet" })

    return // think I broke this, need to test
...
function mrSulu(req, res, next) {
    ...
    log.info("For god's sake, Mr. Sulu! Get those shields " + req.params.key)
...
function serviceResponse(req, res, next) {
    ...
    if (d3ckid == bwana_d3ck.D3CK_ID) {
        ...
        if (service == 'VPN') {
            ...
            log.info('VPN service request....')

            log.info(outstanding_requests)
...
//
// talk to a d3ck/user, will it take you as a pal?
//
function friend_request(req, res, next) {
    ...
    if (d3ckid == bwana_d3ck.D3CK_ID) {
        ...
        console.warn('*** faux form post to server by server')

        // faux post from browser
        var req = { }

        req.body   = d3ckid
        req.secret = secret

        create_d3ck_by_ip(req)
...
function friend_response(req, res, next) {
    ...
    if (! req.isAuthenticated()) {
        ...
        // don't need to log it, but client might/will want it
        d3ck_response.d3ck_data    = _tmp_d3ck
    ...
    // lost cause
        ...
        log.error('not allowed to try friend request')
        res.send(420, {error : 'buzz off'});
    }
...
// setup a tcp proxy
server.get('/setTCPproxy', auth, setTCPproxy)

// setup a tcp proxy
server.get('/setUDPproxy', auth, setUDPproxy)
...
io_sig.on('request', function(req, res) {
